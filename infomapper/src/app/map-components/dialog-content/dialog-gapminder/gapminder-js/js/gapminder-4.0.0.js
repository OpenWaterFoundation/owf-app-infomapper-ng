...
import $ from "jquery";
import * as d3 from 'd3'; // updated
import 'select2'; //updated
...
export function gapminder(configPath) {
  ...
  slider.append("line")
    ...
    .on("mousemove", function (event) { // updated: event passed as first argument to all listeners
      ...
      if (date <= demensions.maxPopulatedDate) {
        slider_tooltip.html(handleFormat(date))
          .style("left", (event.pageX - 30) + "px") // Updated: d3.event ⇨ (event)
          .style("top", (event.pageY - 35) + "px"); // Updated: d3.event ⇨ (event)
      } else {
    ...
    .call(d3.drag()
      ...
      .on("start drag", function (event) { draggedYear(timeScale.invert(Math.round(event.x))); })); // Updated: d3.event ⇨ (event)
  ...
  function add_legend(data) {
    ...
    var square = legend.append("g")
      ...
      .attr("cursor", "pointer")
      .on("mousedown", function (event, d) { // updated: event passed as first argument to all listeners
        if (event.ctrlKey) { // Updated: d3.event ⇨ (event)
          if (firstClick) {
    ...
    var legendText = legend.append("g")
      ...
      .style("cursor", "pointer")
      .on("mousedown", function (d) { // updated: event passed as first argument to all listeners
        if (event.ctrlKey) { // Updated: d3.event ⇨ (event)
          if (firstClick) {
            selectMultiple = false;
            legendButton(d, selectMultiple);
  ...
  function add_path(data) {
    path = svg.append("g")
      ...
      .attr("id", function (d) {
        return "T" + convert_to_id(d[1][0].color); // Updated: d.values changes to d[1]
      })
      .style("stroke", function (d) {
        return colorScale(d[1][0].color); // Updated: d.values changes to d[1]
      })
      ...
      .attr("d", function (d) {
        return line(d[1]); //Updated: d[1] = values
      })
  ...
  function add_dots(data) {
    ...
    dot = dot_g.selectAll(".dot")
      ...
      .on("contextmenu", function (event, d, i) { // Updated: event passed as first argument to all listeners
        if (properties.TimeseriesEnabled) {
          event.preventDefault(); // Updated: d3.event ⇨ (event)
          div.style("opacity", 1);
          div.html("<p style='margin:0px;'><a style='color:black; font-weight:bold;' href='./highchart.html?csv=" + expand_parameter_value(properties.FilePropertyName, { "Year": properties.DefaultDatasetChoice }) + "&name=" + d.name + "&nameVar=" + variables.Label +
            ...
            .style("left", (event.pageX) - 300 + "px") // Updated: d3.event ⇨ (event)
            .style("top", (event.pageY) - 65 + "px"); // Updated: d3.event ⇨ (event)
        }
      })
   ...
   */
  function mousedown(event, d, i) { // Updated: event passed as first argument to all listeners
    if (event.button === 0) { // Updated: d3.event ⇨ (event)
  ...
  */
  ...
  window.updatePath = function (newData) {
    ...
    d3.select("#dataline").selectAll("path")
      ...
      .attr("d", function (d) {
        return line(d[1]); //Updated: d.values changes to d[1]
      });
  }
  ...
  function getGroupingNames(json) {
    ...
    // console.log("varialbles.grouping: ", variables.Grouping);
  ...
  function resize() {
    ...
    if (tracer) {
      ...
      path.attr("d", function (d) {
        return line(d[1]); // Updated: data value accessed using d[1]
      });
    }
...
//  *Displays a tooltip with the annotation information at mouseover event
//  */
function mouseoverAnnotation(event, d) { // Updated: event passed as first argument to all listeners
  tip.transition().duration(0);
  tip.style('top', (event.pageY - 20) + 'px') // Updated: d3.event ⇨ (event)
    .style('left', (event.pageX + 13) + 'px') // Updated: d3.event ⇨ (event)
    .style('display', 'block')
    .html(d.Annotation);
}
